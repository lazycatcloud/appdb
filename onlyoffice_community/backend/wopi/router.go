...
func (s *Server) discover(ctx *gin.Context) {
	bytes, err := fetch_discover_content(s.collaboraHostInner + "/hosting/discovery")
...
func (s *Server) checkInfo(ctx *gin.Context) {
	...
	ctx.JSON(http.StatusOK, gin.H{
		...
		"UserFriendlyName":        metaInfo.UID,
		"IsAnonymousUser":         false,
	})
}
func (s *Server) saveAs(ctx *gin.Context) {
	if ctx.Request.Header.Get("X-Wopi-Fileconversion") == "true" {
		var (
			accessToken = ctx.Query("access_token")
			id          = ctx.Param("id")
		)
		...
		metaInfo.FilePath = metaInfo.FilePath + ctx.GetHeader("X-Wopi-Suggestedtarget")
		metaInfo, _ = auth.Create(metaInfo.UID, metaInfo.FilePath)

		fmt.Printf("%v\n", gin.H{
			"Url":  fmt.Sprintf("https://%s/wopi/files/%s?access_token=%s", ctx.GetHeader("X-Forwarded-Host"), metaInfo.FileId, metaInfo.Token),
			"Name": filepath.Base(metaInfo.FilePath),
		})
		ctx.JSON(http.StatusOK, gin.H{
			"Url":  fmt.Sprintf("https://%s/wopi/files/%s?access_token=%s", ctx.GetHeader("X-Forwarded-Host"), metaInfo.FileId, metaInfo.Token),
			"Name": filepath.Base(metaInfo.FilePath),
...
func (s *Server) active(ctx *gin.Context) {
	...
	req, err := http.NewRequest(http.MethodGet, s.collaboraHostInner+"/welcome/", nil)
	if err != nil {
	...
	res, err := c.Do(req)
	if err != nil {
		ctx.JSON(http.StatusOK, gin.H{"message": "Collabora 正在启动", "err": err.Error()})
		return
	}
	defer res.Body.Close()
	ioutil.ReadAll(res.Body)
	if res.StatusCode != http.StatusOK {
		ctx.JSON(http.StatusOK, gin.H{"message": "Collabora 正在启动", "status": res.Status})
		return
	}
