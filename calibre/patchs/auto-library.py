...
def main():
    ...
    if not auto_lib.need_set_library():
        pass
    elif auto_lib.check_for_existing_library():
...
class AutoLibrary:
    ...
    def need_set_library(self):
        if not os.path.exists(self.app_db):
            return True
        ...
        try:
            ...
            for row in cur.fetchall():
                if os.path.exists(os.path.join(row[0],"metadata.db")):
                    self.lib_path = row[0]
                    return False
        except Exception as e:
            print("[auto-library]: ERROR: Could not Query config_calibre_dir Calibre Web Database")
            print(e)
            return True
