...
runTemplatePlusFile=/wiz/storage/index/.runTemplatePlusFile
...
if [ "$isUbuntu" = true ]; then
  mkdir /nonexistent
  cp -fr /wiz/config/mysqld.cnf /etc/mysql/mariadb.conf.d/mysqld.cnf
...
#  if [ "$isUbuntu" = true ]; then
#    mysql_install_db --user=root --basedir=/wiz/storage/db
#  else
...
pm2 start app.js --name="chrome" --log-date-format="YYYY-MM-DD HH:mm:ss.SSS" -f -- -s chrome
...
if [ ! -f "$runOnceFile" ]; then
  ...
  NODE_ENV=production node import_online_templates.js 
  date  > $runOnceFile
fi

# 初始化模版数据
if [ ! -f "$runTemplatePlusFile" ]; then
  echo "----------init templatePlus-------------"
  /wiz/app/wait-for-it.sh -h 127.0.0.1 -p 5001 -t 60
  asService=$?
  if [[ $asService != 0 ]]; then
    echo "as service start failed";
  ...
  /wiz/app/wait-for-it.sh -h 127.0.0.1 -p 9000 -t 60
  editorService=$?
  if [[ $editorService != 0 ]]; then
    echo "editor service start failed";
    exit;
  fi
  NODE_ENV=production node import_online_templates.js 
  date  > $runTemplatePlusFile
fi
