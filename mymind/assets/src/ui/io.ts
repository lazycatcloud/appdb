...
export function init() {
	[Local, Firebase, GDrive, File, HomeCloud, WebDAV, Image].forEach(ctor => {
		let bui = new ctor();
		select.append(bui.option);
	});

	select.value = localStorage.getItem(`${PREFIX}.backend`) || "homecloud";
...
export function restore() {
	...
	if ("file" in parts && !("b" in parts)) { parts.b = "homecloud"; }
