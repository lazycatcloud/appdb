(() => {
  ...
  // .js/backend/homecloud.js
  var HomeCloud = class extends Backend {
    constructor() {
      super("homecloud");
    }
    save(data, url) {
...
${text}`);
  ...
  // .js/ui/backend/homecloud.js
  var HomeCloudUI = class extends BackendUI {
    constructor() {
      console.log("constructor 1");
      super(new HomeCloud(), "\u61D2\u732B\u4E91");
      this.homeUrl = "/_lzc/files/home";
      this.file = "";
      fill(this.format);
      this.format.value = localStorage.getItem(this.prefix + "format") || "native";
    ...
    get dir() {
      return this.node.querySelector(".home_cloud_save_dir");
    }
    getState() {
    ...
    async load(file = "") {
      ...
      if (file) {
        ...
        try {
          ...
          let format = getByName(`${this.homeUrl}/${this.file}`) || repo6.get("native");
        ...
        let file2 = document.getElementById("home_cloud_file_picker");
        if (file2) {
          ;
          window.invoke(file2, "init");
          window.invoke(file2, "open");
        } else {
...
${text}`);
          ...
          return new Promise((resolve, reject) => {
            canvas.toBlob((blob) => {
              if (blob != null) {
                ...
                reject(new Error("Can not create null blob img."));
              }
            }, "image/png");
          });
          break;
  ...
  function init11() {
    [LocalUI, FirebaseUI, GDriveUI, FileUI, HomeCloudUI, WebDAVUI, ImageUI].forEach((ctor) => {
      let bui = new ctor();
      select6.append(bui.option);
    });
    select6.value = localStorage.getItem(`${PREFIX}.backend`) || "homecloud";
  ...
  function onDone(_message, publisher) {
    hide2();
    console.log("onDone: setCurrentBackend:", _message, publisher);
  ...
  function quickSave() {
    console.log("Quick Save:", currentBackend);
    if (currentBackend) {
      currentBackend.save();
    } else {
      show("save");
  ...
  function setCurrentBackend(backend) {
    console.log("setCurrentBackend", backend);
